$(() => {
  let rowClickedOrder = event => {
    let orderId = $(event.currentTarget)
      .children()
      .first()
      .children()
      .data("id");
  ...
  let rowClickedNotPaidHonorary = event => {
    let translatorId = $(event.currentTarget)
      .children()
      .first()
      .children()
      .data("id");
  ...
  let rowClicketNotPaidOrdersByCustomer = event => {
    let customerId = $(event.currentTarget)
      .children()
      .first()
      .children()
      .data("id");
  ...
  let callbacks = {
    rowCallbackOrder: row => {
      ...
      $(row)
        .find("td:nth-child(7),td:nth-child(8)")
        .click(event => {
              ...
              setTimeout(() => {
                if (res.isDone) {
                  if (JSON.parse(res.isDone)) {
                    ...
                    $(event.target).html(
                      '<span class="not-done">Не е готова</span>'
                    );
                  }
                }
                ...
                if (res.isPaid) {
                  if (JSON.parse(res.isPaid)) {
                    ...
                    $(event.target).html(
                      '<span class="not-done">Не е платена</span>'
                    );
                  }
                }
              }, 10);
    ...
    footerCallback: function(row, data, start, end, display) {
      ...
      // Update footer
      $(api.column(1).footer()).html(`${total} лв.`);
    },
    footerCallbackNotPaidByCustomer: function(row, data, start, end, display) {
      ...
      // Update footer
      $(api.column(11).footer()).html(`${total} лв.`);
    },
  ...
  $("table[data-table-ajax]").dataTable({
    ...
    rowCallback:
      callbacks[$("table[data-table-ajax]").attr("data-table-row-callback")],
    drawCallback:
      callbacks[$("table[data-table-ajax]").attr("data-table-draw-callback")],
    footerCallback:
      callbacks[$("table[data-table-ajax]").attr("data-table-footer-callback")],
    pageLength: 100,
    order: $("table[data-table-ajax]").attr("data-table-sort")
      ? JSON.parse($("table[data-table-ajax]").attr("data-table-sort"))
      : ""
  });
